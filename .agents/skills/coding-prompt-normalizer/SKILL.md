...
- Do not "improve" or rename tokens like
  ...
  `npx @gonkagate/mimo-code-setup`, `provider.gonkagate`,
  ...
  `docs/specs/mimo-code-setup-prd/spec.md`, or
  `docs/plans/mimo-code-setup-implementation-plan.md`.
...
`AGENTS.md`, `docs/`, `CHANGELOG.md`, or keeping the scaffolded installer
contract truthfully described.
...
`test/skills-contract.test.ts` so the skill is MiMoCode-specific, preserves
`~/.config/mimocode/mimocode.json`, and removes stale Codex wording. Keep the
...
skill to `mimo-code-setup`, preserving `~/.config/mimocode/mimocode.json` as an
...
MiMoCode-specific ones, update any stale literals or examples, and add the
...
does around `npx @gonkagate/mimo-code-setup` and `chat_completions`. Need
