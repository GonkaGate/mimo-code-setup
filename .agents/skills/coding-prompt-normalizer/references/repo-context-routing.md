...
- current transport target is `chat_completions`
...
`~/.config/mimocode/mimocode.json`, `mimocode.json`, `provider.gonkagate`,
...
- `src/install/` does not exist yet
- config and provider rules currently live in docs, tests, and constants
- prompts should not assume runtime modules before they are created
...
- docs should distinguish scaffolded installer behavior from future product
  intent
...
- repository tests protect scaffold, package, skills, and doc-contract
  expectations
...
- if a skill is repo-specific, examples and literals should point to MiMoCode
