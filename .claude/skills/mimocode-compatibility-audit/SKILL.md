---
name: mimocode-compatibility-audit
description: "Read-only compatibility audit between `mimo-code-setup` and the latest stable `@mimo-ai/cli` release plus official MiMoCode docs. Use whenever the task is to decide whether this repository still matches current MiMoCode config, custom-provider, auth, model, or CLI contracts, or whether upstream MiMoCode changed in a way that breaks our setup plan, even if the user only asks 'is this still compatible?' or 'did MiMoCode upstream change?'."
---
...
is `mimo-code-setup` still compatible with the current stable upstream MiMoCode
contract or not?

This is a read-only compatibility gate. The job is to compare official
upstream MiMoCode behavior against the assumptions encoded in this repository
and return a clear verdict, not to design or apply a migration.

## Scope

Cover the repository's current and planned MiMoCode-facing contract,
especially:

- config location, merge order, and precedence assumptions for
  `~/.config/mimocode/mimocode.json`, `MIMOCODE_CONFIG`,
  `MIMOCODE_CONFIG_CONTENT`, and project `mimocode.json`
...
- custom-provider wiring through `provider.<id>`, including custom provider
  ...
  `options.headers`
- model selection assumptions around `model`, `small_model`, `mimo models`,
  and `provider/model` identifiers
- auth strategy assumptions around `/connect`, `mimo providers login`,
  `~/.local/share/mimo/auth.json`, and the repository's decision not to use
...
- workflow and CLI assumptions documented by this repository, such as
  `mimo`, `mimo run`, `mimo models`, and
  `mimo providers login`
- newly required settings, renamed fields, removed commands, or release-level
  behavior changes that would make the documented GonkaGate MiMoCode plan stale
  or unsafe

Default compatibility target:

- latest stable `@mimo-ai/cli` release from the npm `latest` dist-tag
...
- broaden product scope beyond the current GonkaGate MiMoCode contract
...
- npm registry metadata for `@mimo-ai/cli`
- official MiMoCode docs, especially:
  - `https://github.com/XiaomiMiMo/MiMo-Code/`
  - `https://mimo.ai/docs/providers/`
  - `https://mimo.ai/docs/models/`
  - `https://mimo.ai/docs/cli/`
...
1. `npm view @mimo-ai/cli version dist-tags repository.url homepage --json`
...
- `npm view @mimo-ai/cli version dist-tags repository.url homepage --json`
- `curl -fsSL https://mimo.ai/config.json`
- `curl -fsSL https://github.com/XiaomiMiMo/MiMo-Code/`
- `curl -fsSL https://mimo.ai/docs/providers/`
- `npx -y @mimo-ai/cli@<version> --help`
- `npx -y @mimo-ai/cli@<version> models --help`
- `npx -y @mimo-ai/cli@<version> providers login --help`
...
- Prefer docs, schema, release notes, CLI help, source, and tests over running
  stateful commands.
- Never run upstream MiMoCode commands against the user's real
  `~/.config/mimocode`, `~/.local/share/mimo`, or project config.
- If you need CLI help or read-only behavior inspection, isolate it in a
  ...
  `XDG_DATA_HOME`, `MIMOCODE_CONFIG`, and any other relevant config roots at
  temp paths.
...
- whether newer prerelease channels exist and whether they signal upcoming
  contract drift
- where MiMoCode loads global config from and how project `mimocode.json`
...
- whether custom-provider auth still relies on `/connect` or
  `mimo providers login` storage plus config, and whether `auth.json`
...
- whether current custom-provider guidance still recommends
  ...
  for `/v1/responses`
- whether MiMoCode added or removed CLI surfaces relevant to this repository's
  documented flow
...
1. Identify the audit target.
   - Determine the latest stable `@mimo-ai/cli` release from npm metadata.
...
4. Compare the critical seams one by one.
   - `Config locations and precedence`
     ...
     `~/.config/mimocode/mimocode.json`, `MIMOCODE_CONFIG`,
     `MIMOCODE_CONFIG_CONTENT`, and `mimocode.json` assumptions.
   - `Provider wiring`
...
6. Decide the verdict.
   ...
   - `incompatible`
     ...
     assumption or makes the documented GonkaGate MiMoCode plan stale or
     unsafe.
